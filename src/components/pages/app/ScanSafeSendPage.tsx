...
export function ScanSafeSendPage() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (i < ORDER.length) {
        timers.push(setTimeout(tick, 700 + Math.random() * 600));
      } else {
    ...
    <div className="surface-card p-7">
      ...
      <div className="mt-7 space-y-1.5">
        {ORDER.map((s, i) => {
          ...
          return (
            <div key={s} className="flex items-center justify-between rounded-lg border border-[var(--border)] bg-surface-elevated/70 px-4 py-2.5">
              <div className="flex items-center gap-3">
                <span className={`inline-flex h-1.5 w-1.5 rounded-full transition ${done ? "bg-cyan shadow-[0_0_10px_oklch(0.82_0.13_210)]" : scanning ? "animate-pulse-soft bg-amber" : "bg-[var(--border-strong)]"}`} />
        ...
        <div className="mt-2 max-h-40 space-y-1 overflow-hidden font-mono text-[11.5px]">
          {logs.map((l, i) => (
            <div key={i} className="flex gap-3">
              ...
              <span className={l.lvl === "block" ? "text-red" : l.lvl === "warn" ? "text-amber" : l.lvl === "info" ? "text-cyan" : "text-emerald"}>
                [{l.lvl}]
              </span>
